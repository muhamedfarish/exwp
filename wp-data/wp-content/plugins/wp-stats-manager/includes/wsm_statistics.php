			...
			foreach( $arrVisitorsInfo as $key => $data ){
				...
				if(count($data) > 0)
				{
					foreach( $data as $data_key => $row ){
						...
						$contentData.='<tr id="wsmRowParent_'.esc_attr($counter).'" class="wsmReferralRow"><td><div class="wsmTooltip">'.$arrow.'&nbsp;'.$counter.$toolTip.'</div></td><td><a href="#" data-referrak_param=\''.(json_encode($arrAtts)).'\' class="linkReferralVisitorDetails" title="'.__('Click here for details.','wp-stats-manager').'">'.$row['name'].'</a></td><td></td><td>'.number_format($row['total']).$compareValue.'</td></tr>';
