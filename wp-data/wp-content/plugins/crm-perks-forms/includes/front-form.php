   ...
   if(typeof filter == 'object'){
       jQuery.each(filter,function(k,box){
         ...
         jQuery.each(box,function(k,v){
          if(v['field'] != ''){
          val=jQuery('#crm_field_'+form_id+'_'+v['field'],body).val(); 
...
jQuery('<?php echo esc_attr($trigger); ?>').click(function(e){
  e.preventDefault();
 cfx_open_box_btn($('#cfx_round_btn_<?php echo $form_id; ?>'));   
});
<?php } ?>   
})
</script>
...
$field_val="<div><a href='$field_val' target='_blank'>".$file_name."</a></div>";
     }
