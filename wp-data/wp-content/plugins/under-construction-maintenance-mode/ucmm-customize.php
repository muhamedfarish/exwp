...
$ucmm_schedule_end_time = ucmm_wpbrigade_default_option_key( 'ucmm_schedule_end', $ucmm_wpbrigade_array );
$ucmm_show_end_time     = ucmm_wpbrigade_default_option_key( 'ucmm_schedule_show_end_time', $ucmm_wpbrigade_array );
...
var x = setInterval(function() {
  ...
  if (distance < 0) {
	clearInterval(x);
	document.querySelector('.ucmm_schedule_time').innerHTML = 
	"<?php 
		if ( ! is_customize_preview() ) {
			esc_html_e( 'Website is now LIVE! will be redirected to homepage shortly. If not, please refresh the page.', 'ucmm-wpbrigade' );
			
		} else{
			esc_html_e( 'Maintenance schedule has been expired. Please update the time or turn if off. ', 'ucmm-wpbrigade' );
			}
	?>";
	if( distance == -1000 ) {
